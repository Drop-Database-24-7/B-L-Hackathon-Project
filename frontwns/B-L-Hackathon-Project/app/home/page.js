...
export default function Home() {
  ...
  const handleShowClick = async () => {
    ...
    const name = sattellites.above.find(item => item.satid == currentSat).satname;
    ...
    try {
      ...
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        headers: {
          ...
          Authorization: `{wirte a key}` //its in a file ignored by git
        },
        body: JSON.stringify({
          model: "gpt-4o-mini",
          messages: [
            { role: "system", content: "You are a helpful assistant providing satellite information." },
            { role: "user", content: `Provide short information about the satellite: ${name}.` }
          ]
        })
      });
